...
export class BookmarkParser {
  constructor(private browserType: BrowserType) {}
  ...
  private parseFolder(element: Element, parentTitle?: string): BookmarkFolder {
    ...
    const addDate = parseInt(
      element.querySelector("h3")?.getAttribute("ADD_DATE") || "0"
    );
    const lastModified = parseInt(
      element.querySelector("h3")?.getAttribute("LAST_MODIFIED") || "0"
    );
  ...
  public parseHtmlFile(htmlContent: string): BookmarkTree {
    ...
    return { root, browserType: this.browserType };
  }
}
